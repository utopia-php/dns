...
final class RecordTest extends TestCase
{
    ...
    public function testEncodeSoaRecordPreservesPreEscapedRnameWithoutAt(): void
    {
        $record = new Record(
            name: 'example.com',
            type: Record::TYPE_SOA,
    ...
    public function testEncodeSoaRecordWireFormatRoundTripsEmailRname(): void
    {
        $record = new Record(
            name: 'example.com',
            type: Record::TYPE_SOA,
        ...
        // After wire decode the dotted RNAME is no longer distinguishable from
        // an "ordinary" three-label name, so the local part is parsed as a
        // separate label. This pins the current encode/decode asymmetry
        // documented on Domain::decode().
        $this->assertSame('example.com', $decoded->name);
        $this->assertSame(Record::TYPE_SOA, $decoded->type);
        $this->assertStringContainsString('hostmaster.example.com', $decoded->rdata);
    }

    public function testEncodeSoaRecordRejectsRnameWithMultipleAt(): void
    {
        $record = new Record(
            name: 'example.com',
            type: Record::TYPE_SOA,
            class: Record::CLASS_IN,
            ttl: 3600,
            rdata: 'ns1.example.com foo@bar@example.com 2024102701 7200 3600 1209600 86400'
        );

        $this->expectException(\InvalidArgumentException::class);
        $this->expectExceptionMessage('exactly one @ separator');

        $record->encode();
    }

    public function testEncodeSoaRecordRejectsRnameWithEmptyLocalPart(): void
    {
        $record = new Record(
            name: 'example.com',
            type: Record::TYPE_SOA,
            class: Record::CLASS_IN,
            ttl: 3600,
            rdata: 'ns1.example.com @example.com 2024102701 7200 3600 1209600 86400'
        );

        $this->expectException(\InvalidArgumentException::class);
        $this->expectExceptionMessage('non-empty local part and domain');

        $record->encode();
    }

    public function testEncodeSoaRecordRejectsRnameWithEmptyDomain(): void
    {
        $record = new Record(
            name: 'example.com',
            type: Record::TYPE_SOA,
            class: Record::CLASS_IN,
            ttl: 3600,
            rdata: 'ns1.example.com hostmaster@ 2024102701 7200 3600 1209600 86400'
        );
    ...
    public function testEncodeSoaRecordRejectsRnameWithTrailingBackslashInLocalPart(): void
    {
        $record = new Record(
            name: 'example.com',
            type: Record::TYPE_SOA,
            class: Record::CLASS_IN,
            ttl: 3600,
            rdata: 'ns1.example.com user\\@example.com 2024102701 7200 3600 1209600 86400'
        );

        $this->expectException(\InvalidArgumentException::class);
        $this->expectExceptionMessage('dangling backslash');

        $record->encode();
    }

    public function testEncodeSoaRecordAcceptsSingleLabelRname(): void
    {
        $record = new Record(
            name: 'example.com',
            type: Record::TYPE_SOA,
            class: Record::CLASS_IN,
            ttl: 3600,
            rdata: 'ns1.example.com hostmaster@localhost 2024102701 7200 3600 1209600 86400'
        );

        $encoded = $record->encode();

        $this->assertStringContainsString("\x0Ahostmaster\x09localhost\x00", $encoded);
    }

    public function testEncodeSoaRecordRejectsRnameWithLocalPartOverLabelLimit(): void
    {
        ...
        $record = new Record(
            ...
            class: Record::CLASS_IN,
            ttl: 3600,
            rdata: "ns1.example.com {$localPart}@example.com 2024102701 7200 3600 1209600 86400"
        );
